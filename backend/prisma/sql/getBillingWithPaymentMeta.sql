...
-- @param {Boolean} $6:isDeleted If the retrieved items are deleted or not
...
FROM (
    ...
    WHERE (
        $1::"PaymentScheme" IS NULL
        OR bill."paymentScheme" = $1::"PaymentScheme"
      )
      AND (
        $2::"BillType" IS NULL
        OR bill."billType" = $2::"BillType"
      )
      AND (
        $5::uuid IS NULL
        OR bill."userId" = $5::uuid
      )
      AND (
        $4::text IS NULL
        OR (
